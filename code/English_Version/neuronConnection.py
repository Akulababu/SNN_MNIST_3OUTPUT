"""
Neural Network Connection Analysis Module (Forward Direction)
This code analyzes the neural network connections in forward direction with features:
- Loads weight matrices from trained model
- Analyzes neuron connections from input to output
- Identifies dead neurons in each layer
- Calculates fan-out statistics and connection signs
...
def process_matrices(weight_matrix_layer0, weight_matrix_layer2):
    with open('neuron_connections.txt', 'w', encoding='utf-8') as output_file:
        ...
        # Process Layer 0 (forward direction)
        ...
        # Process Layer 2 (forward direction)
        ...
        # Layer 0 dead neurons (no output connections)
        dead_neurons_layer0 = []
        ...
        # Layer 2 dead neurons (no input connections)
        dead_neurons_layer2 = []
        for i in range(weight_matrix_layer2.shape[0]):
