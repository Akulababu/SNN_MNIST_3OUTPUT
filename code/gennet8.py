...
class QuantLayer(nn.Module):
    ...
    def print_weight_stats(self, w, is_quantized=False):
        ...
        if not is_quantized:
            ...
            print(f"\n层级权重统计 ({self.out_features}x{self.in_features}):")
            print(f"未量化权重分布:")
            print(f"最小值: {w.min().item():.4f}")
            print(f"最大值: {w.max().item():.4f}")
            print(f"平均值: {w.mean().item():.4f}")
            print(f"标准差: {w.std().item():.4f}")
            print(f"非零权重比例: {nonzero/total_weights:.4f}")
        else:
            # 只在中文或英文模式下打印一次
            if not hasattr(self, '_stats_printed'):
                unique_values = torch.unique(w)
                print(f"\n层级权重统计 ({self.out_features}x{self.in_features}):")
                print(f"量化后权重分布 (阈值: {self.threshold:.4f}):")
                
                sorted_values = sorted(unique_values.tolist(), key=abs)
                for val in sorted_values:
                    count = (w == val).sum().item()
                    print(f"  值 {val:6.3f}: {count:6d} ({100*count/total_weights:6.2f}%)")
                
                nonzero = (w != 0).sum().item()
                print(f"非零权重比例: {nonzero/total_weights:.4f}")
                self._stats_printed = True  # 标记已打印
    ...
    def normalize_weights(self):
        ...
        with torch.no_grad():
            ...
            print(f"\n层 {self.layer_index} 权重标准化完成:")
            print(f"均值: {self.weight_mean:.4f}, 标准差: {self.weight_std:.4f}")
            print(f"标准化后范围: [{normalized_weights.min():.4f}, {normalized_weights.max():.4f}]")
    '''
    ...
    def forward(self, x):
        # 在量化开始前进行标准化
        if not self.is_normalized and not self.enable_quantization and self.epoch_counter == self.quant_start_epoch - 1:
    ...
    def on_epoch_end(self):
        ...
        if self.epoch_counter == self.quant_start_epoch - 1:
            print(f"\n准备开始量化 - Layer {self.out_features}x{self.in_features}")
            self.normalize_weights()
        
        # 先处理剪枝
        if (self.prune_config is not None and 
            self.epoch_counter == self.prune_config['start_epoch']):
            print(f"\n开始剪枝 - Layer {self.out_features}x{self.in_features}")
            self.update_mask()
            # 打印剪枝后的统计信息
        ...
        if not self.enable_quantization and self.epoch_counter >= self.quant_start_epoch:
            print(f"\n开启量化 - Layer {self.out_features}x{self.in_features}")
...
class QuantSNNNetwork(nn.Module):
    def __init__(self, num_inputs, num_hidden, num_outputs, beta, threshold, 
        ...
        # 添加这一行来初始化 layers 列表
        self.layers = nn.ModuleList()  # 使用 ModuleList 而不是普通的 list
            
        delay_between_layers = 15
        total_layers = len(num_hidden) + 1
        
        # 第一层 - 最后量化
        self.layers.append(QuantLayer(num_inputs, num_hidden[0], 
                                   prune_config=pruning_config.get(0),
                                   quant_start_epoch=quant_start_epoch + (total_layers-1)*delay_between_layers,
                                   layer_index=0))
        ...
        self.layers.append(snn.Leaky(beta=beta[0],
                                  ...
                                  reset_mechanism='zero',  # 修改为zero
                                  init_hidden=True,
                                  output='both'))
        
        # 隐藏层
        for i in range(1, len(num_hidden)):
            layer_quant_start = quant_start_epoch + (total_layers-i-1)*delay_between_layers
            self.layers.append(QuantLayer(num_hidden[i-1], num_hidden[i],
                                       prune_config=pruning_config.get(i),
                                       quant_start_epoch=layer_quant_start,
                                       layer_index=i))
            self.layers.append(snn.Leaky(beta=beta[i],
        ...
        # 输出层 - 最先开始量化
        self.layers.append(QuantLayer(num_hidden[-1], num_outputs,
                                   prune_config=pruning_config.get(len(num_hidden)),
                                   quant_start_epoch=quant_start_epoch,
                                   layer_index=len(num_hidden)))
    ...
    def forward(self, x):
        ...
        for i, layer in enumerate(self.layers):
            if isinstance(layer, QuantLayer):
                ...
                curr_x, curr_mem = layer(curr_x)
        return curr_x, curr_mem  # 返回脉冲和膜电位
