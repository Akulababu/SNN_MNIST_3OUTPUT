...
class MNISTDataProcessor:
    ...
    def preprocess_batch(batch): 
        ...
        for i in range(batch_size):
            ...
            if i % 100 == 0:
                ...
                print(f"Batch {i} activation rate: {activation_rate:.3f}")
    ...
    def print_batch_statistics(self, batch_data, batch_labels):
        ...
        """
        print("样本批次形状:")
        print(f"数据: {batch_data.shape}")
        print(f"标签: {batch_labels.shape}")
        
        print("\n数据统计:")
        print(f"数据均值: {batch_data.mean():.4f}")
        print(f"数据标准差: {batch_data.std():.4f}")
        print(f"0的数量: {(batch_data == 0).sum().item()}")
        print(f"1的数量: {(batch_data == 1).sum().item()}")
        print(f"\n标签分布:")
        for i in range(6):  # 只显示0-5的类别
            count = (batch_labels == i).sum().item()
            print(f"类别 {i}: {count} 个样本 ({100 * count/len(batch_labels):.1f}%)")
...
def verify_preprocessing(processor):
    ...
    for batch, labels in processor.train_loader:
        ...
        # 打印统计信息
        activation_rate = processed.float().mean().item()
        print(f"平均激活率: {activation_rate:.3f}")
        print(f"每个样本的激活数量: {processed.sum(dim=1).int().tolist()[:5]}")  # 显示前5个样本
        
        # 可视化几个样本
        for i in range(3):  # 显示前3个样本
            sample = processed[i].view(7, 7)
            print(f"\n样本 {i}:")
            for row in sample:
                print(" ".join("1" if x > 0.5 else "0" for x in row))
        break
