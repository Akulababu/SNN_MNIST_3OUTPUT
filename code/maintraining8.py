...
# 初始化模型参数
num_inputs = 49  
num_hidden = [24]  # 移除了12，只保留24
num_outputs = 3   
beta_values = [0,0,0]  # 减少为3个值
thresholds = [1, 2, 2]  # 减少为3个值

# 定义量化开始的epoch
quant_start_epoch = 70 # 或其他你想要的值
...
optimizer_fn = lambda params: torch.optim.AdamW(
    params, # 要优化的网络参数
    lr=0.005,  # 稍微提高初始学习率
    weight_decay=0.0001, # L2正则化系数：用于防止过拟合，这里设为0表示不使用
    betas=(0.9, 0.999) , # 动量系数：用于计算梯度的移动平均
                        # 第一个值(0.8)控制一阶矩估计
                        # 第二个值(0.999)控制二阶矩估计
)
...
'''
def combined_loss_with_constraints(spikes, mem_potentials, targets, model, constraint_weight=0.01):
    """
    ...
    Returns:
        组合后的损失值
    """
    spike_loss = nn.CrossEntropyLoss(label_smoothing=0.05)(spikes, targets)
    ...
    for layer in model.layers:
        if isinstance(layer, QuantLayer):
            ...
            positive_weights = (weights > 0).float()
            negative_weights = (weights < 0).float()
   
            # Calculate excess positive and negative weights
            excess_positive = F.relu(positive_weights.sum(dim=1) - layer.max_positive_weights)
            excess_negative = F.relu(negative_weights.sum(dim=1) - layer.max_negative_weights)
   
            # Sum of squared excesses
            constraint_loss += (excess_positive ** 2).mean() + (excess_negative ** 2).mean()
   
    total_loss = 0.6 * spike_loss + 0.4 * mem_loss + constraint_weight * constraint_loss
    return total_loss
...
train_model(
    ...
    eval_interval=1,
    scheduler_config=scheduler_config,  # 传入scheduler配置
    optimizer_fn=optimizer_fn,
)
